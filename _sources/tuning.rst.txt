...
- R/Q must be tuned with a cell-local handle, never ``Req``. ``Req`` is the *shared* equator radius; moving it to change one cell's R/Q detunes every other cell. Frequency is set by ``Req`` (shared) or the end length ``L`` (local); R/Q by the equator ellipse semi-axis ``A`` (local).
...
See :doc:`examples/tuning/qoi_targets` for all of this worked through: the unique design, the over- and under-constrained cases, the cup additivity measured, and the staged tune with its no-solution report.
