...
class WakefieldSolver:
    ...
    def plot_z(self, ax=None, quantity='impedance'):
        """Plot the longitudinal wake result. ``quantity`` is 'impedance'
        (|Z_L| vs f) or 'wake' (wake potential vs s)."""
        return self._plot(self.wake_z, ax, quantity,
                          impedance_cols=('f [MHz]', '|Z| [Ohm]'),
                          wake_cols=('s [m]', 'W [V/pC]'),
                          ylabels=(r'$|Z_\parallel|$ [$\Omega$]', r'$W_\parallel$ [V/pC]'),
                          title='Longitudinal')

    def plot_t(self, ax=None, quantity='impedance'):
        """Plot the transverse wake result. ``quantity`` is 'impedance'
        (|Z_T| vs f) or 'wake' (wake potential vs s)."""
        return self._plot(self.wake_t, ax, quantity,
                          impedance_cols=('f [MHz]', '|Z| [Ohm/m]'),
                          wake_cols=('s [m]', 'W [V/pC/m]'),
                          ylabels=(r'$|Z_\perp|$ [$\Omega$/m]', r'$W_\perp$ [V/pC/m]'),
                          title='Transverse')

    @staticmethod
    def _plot(df, ax, quantity, impedance_cols, wake_cols, ylabels, title):
